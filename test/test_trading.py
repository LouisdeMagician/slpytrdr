...
SELL_BACKOFF = int(os.getenv("SELL_BACKOFF", "5"))
...
class JupiterTrader:
    ...
    async def execute_buy(self, token_address: str) -> str:
        try:
            ...
            balance_resp = await self.client.get_balance(self.wallet.pubkey(), commitment=Confirmed)
            balance_sol = Decimal(balance_resp.value) / Decimal("1e9")
            if balance_sol < TRADE_AMOUNT_SOL + Decimal("0.01"):  # Add buffer for fees
                raise ValueError(f"Insufficient SOL balance: {balance_sol} < {TRADE_AMOUNT_SOL + Decimal('0.01')}")
    ...
    async def execute_sell_all(self, token_address: str) -> str:
        ...
        while retries > 0:
            try:
                ...
                retries -= 1
                if retries == 0:
                    logger.critical("Sell failed after %d retries for %s: %s", SELL_RETRIES, token_address, str(e), exc_info=True)
                    raise
                logger.warning("Sell attempt failed for %s: %s, %d retries left, retrying in %ds", token_address, str(e), retries, backoff)
        ...
        try:
            ...
            for attempt in range(2):  # 2 attempts per source
                try:
                    ...
                    logger.warning(f"Moralis attempt {attempt + 1} failed: {str(moralis_error)}")
                    if attempt == 1:
                        logger.info("Moralis failed, trying Birdeye")
                        for birdeye_attempt in range(2):
                            try:
                                ...
                                logger.warning(f"Birdeye attempt {birdeye_attempt + 1} failed: {str(birdeye_error)}")
                                if birdeye_attempt == 1:
                                    raise ValueError("Failed to fetch SOL/USD price from both Moralis and Birdeye")
                    await asyncio.sleep(1)  # Brief delay between retries

            if sol_usd is None:
                raise ValueError("No SOL/USD price available after retries")
